...
import { ConfirmMarkCatalog, ConfirmMarkControls, useConfirmMark } from './ConfirmMarkCatalog';
...
import { defaultSceneAnimation, defaultSceneAnimationScene, SceneAnimLab, SceneRegionPicker, SCENE_ANIM_REGIONS, SCENE_ANIM_SCENES, SceneRegionThumb, type SceneRegion, type SceneAnimScene } from './SceneAnimLab';
...
const defaultWallCatalogAsset = (): WallCatalogAsset => {
  const material = defaultWallMaterial();
  const matches = wallCatalogAssets().filter((asset) => asset.material === material);
  if (matches.length !== 1) throw new Error(`wall material default ${material} has ${matches.length} catalog rows`);
  return matches[0];
};
...
const familySample = (family: StudioFamily): StudioAsset => {
  const matches = family.assets.filter((asset) => asset.kind === 'tile' && asset.role === 'base');
  if (matches.length !== 1) throw new Error(`Terrain family "${family.id}" expected one DB-owned base surface, found ${matches.length}`);
  return matches[0];
};
...
const readTilesetStudioRoute = (): TilesetStudioRouteState => {
  ...
  return {
    ...
    selectedGroundCoverId: cover || undefined,
    selectedWallDecorId: wdecor || undefined,
    selectedWallArtId: wart || undefined,
    selectedFenceArtworkId: fenceArt || undefined,
    selectedSurfaceFamily: sfamily || undefined,
    selectedRegionId: regionParam || undefined,
    viewerKind: isCardIconReviewAlias ? 'cardfit' : isUnitStudioAlias ? 'unitart' : isNineSliceAlias ? 'nineslice' : isPropLabAlias || isDoodadEditorAlias ? 'propseat' : isTileCompareAlias ? 'tilecompare' : isSurfaceLabAlias ? 'surfacetiles' : isSceneAnimAlias ? 'sceneanim' : isWallCandidatesAlias ? 'wallcandidates' : isArtworkCompareAlias ? 'artworkcompare'
...
function preserveCardIconFittingRouteParams(params: URLSearchParams, route: TilesetStudioRouteState): void {
  if (route.viewerKind !== 'cardfit') return;
...
function preserveDeploymentLabRouteParams(params: URLSearchParams, route: TilesetStudioRouteState): void {
  if (route.category !== 'deployment' && route.viewerKind !== 'deployment') return;
  const current = new URLSearchParams(window.location.search);
  (['df', 'dr', 'dg', 'ds', 'dl', 'dp', 'dk', 'dmr', 'dsc', 'du', 'do', 'dm'] as const).forEach((key) => {
    const value = current.get(key);
...
export function TilesetStudio({ initialCategory = 'tiles' }: { initialCategory?: StudioCategory } = {}): ReactElement {
  ...
  const [doodadBrushId, setDoodadBrushId] = useState<string>(() => defaultDoodadAsset().id);
  ...
  const [sourceArtSearch, setSourceArtSearch] = useState('');
  ...
  const confirmMark = useConfirmMark();
  ...
  const [selectedChromeLabTargetId, setSelectedChromeLabTargetId] = useState<string>(initialRoute.selectedChromeLabTargetId ?? defaultChromeLabTargetId());
  ...
  const [selectedSourceArtId, setSelectedSourceArtId] = useState(initialRoute.selectedSourceArtId ?? '');
  ...
  const [selectedPropName, setSelectedPropName] = useState(() => initialRoute.selectedPropName ?? defaultPropDef().id);
  // Which pipeline tile the embedded Tile Pipeline compare (Viewer 'tilecompare' kind) shows.
  const [selectedTileCompareId, setSelectedTileCompareId] = useState(initialRoute.selectedTileCompareId ?? defaultCompareTile().id);
  const [selectedGroundCoverId, setSelectedGroundCoverId] = useState<GroundCoverId>(() => initialRoute.selectedGroundCoverId
    ? groundCoverAsset(initialRoute.selectedGroundCoverId).id
    : defaultGroundCoverAsset().id);
  const [selectedWallDecorId, setSelectedWallDecorId] = useState<string>(() => initialRoute.selectedWallDecorId
    ? (wallDecorAsset(initialRoute.selectedWallDecorId)?.id ?? (() => { throw new Error(`wall decoration ${initialRoute.selectedWallDecorId} is unavailable`); })())
    : defaultWallDecorAsset().id);
  const [selectedWallArtId, setSelectedWallArtId] = useState<string>(() => wallArtIdOrDefault(initialRoute.selectedWallArtId));
  ...
  const [selectedWallId, setSelectedWallId] = useState<string>(() => defaultWallCatalogAsset().id);
  // Which family the embedded Tileset Surfaces inspector (Viewer 'surfacetiles' kind) opens on.
  const [selectedSurfaceFamily, setSelectedSurfaceFamily] = useState(initialRoute.selectedSurfaceFamily ?? defaultTerrainFamily().id);
  ...
  const [unitBrushId, setUnitBrushId] = useState<string>(initialRoute.selectedUnitId ?? 'pawn');

  const family = studioFamilies.find((item) => item.id === familyId);
  if (!family) throw new Error(`Selected terrain family "${familyId}" is unavailable`);
  ...
  const [selectedAssetId, setSelectedAssetId] = useState(() => initialRoute.selectedAssetId ?? familySample(family).id);
  ...
  const selectedAsset = allStudioAssets.find((asset) => asset.id === selectedAssetId);
  if (!selectedAsset) throw new Error(`Selected terrain asset "${selectedAssetId}" is unavailable`);
  ...
  useEffect(() => {
    const syncFromRoute = () => {
      ...
      if (route.selectedWallArtId) setSelectedWallArtId(wallArtIdOrDefault(route.selectedWallArtId));
      ...
      setSelectedAssetId(route.selectedAssetId ?? familySample(routeFamily).id);
  ...
  useEffect(() => {
    if (!allStudioAssets.some((asset) => asset.id === selectedAssetId)) {
      setSelectedAssetId(familySample(family).id);
  ...
  const doodadsCatalogType: CatalogType<DoodadAsset> = {
    ...
    onCreate: () => {
      const sourceDoodad = doodadAssets.find((d) => d.id === doodadBrushId);
      if (!sourceDoodad) throw new Error(`Selected doodad "${doodadBrushId}" is unavailable`);
  ...
  const propsCatalogType: CatalogType<PropDef> = {
    ...
    onCreate: () => {
      ...
      if (!sourceProp) throw new Error(`Selected prop "${selectedPropName}" is unavailable`);
  ...
  const catalogCategories: { id: StudioCategory; label: string; hint: string; main: ReactElement; controls: ReactElement }[] = [
    ...
    },
    {
      ...
      controls: (
        <>
          <CatalogControls type={unitsCatalogType} />
          {/* The Unit Roster reviews these same units as a set. Registered and implemented, but
              openable only by typing ?vk= until now — see check-studio-reachability.mjs. */}
          <button type="button" className="tileset-view-action" onClick={() => openViewer('unitroster')}>Open Unit Roster</button>
        </>
      ),
    },
    {
    ...
    {
      id: 'sourceart', label: 'Source Art', hint: 'Review and install complete eight-way Blender source-art turntables.',
      main: (
        <SourceArtTurntableCatalog
          search={sourceArtSearch}
          zoom={zoom}
          selected={selectedSourceArtId}
          onSelect={setSelectedSourceArtId}
          onView={(id) => { setSelectedSourceArtId(id); openViewer('sourceart'); }}
        />
      ),
      controls: (
        <>
          <label className="tileset-catalog-search">
            <span>Search</span>
            <input type="search" value={sourceArtSearch} onChange={(event) => setSourceArtSearch(event.target.value)} placeholder="castle, rock, mill…" />
          </label>
          <label className="tileset-catalog-zoom">
            <span>Zoom</span>
            <input type="range" min="0.75" max="2" step="0.05" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} />
          </label>
          <button type="button" className="tileset-view-action" onClick={() => openViewer('sourceart')}>View Selected</button>
        </>
      ),
    },
    {
    ...
    {
      id: 'logmarks', label: 'Log Marks', hint: 'The mark a Battle Event Log prose line wears when it says the Battle was lost. Every candidate is drawn on the real log rows at the real 18px seat, beside the clock it shares a row with and the coin below it. Install binds the selected one.',
      main: <BattleLogMarkCatalog state={battleLogMark} />,
    ...
    {
      id: 'confirmmark', label: 'Confirm Mark', hint: 'The mark every committing verb wears — Play on a Run you are resuming, Start Run on one you are beginning. One mark for the act, mounted in the real band beside the shortest label it takes and the longest. Install binds all of them.',
      main: <ConfirmMarkCatalog state={confirmMark} />,
      controls: <ConfirmMarkControls state={confirmMark} />,
    },
    {
    ...
    },
    {
      ...
      controls: (
        <>
          ...
          {/* The Loading Lab inspects how these same pages compose on the way in, so it belongs
              beside them. It was registered and implemented but openable only by typing ?vk= —
              see check-studio-reachability.mjs. */}
          <button type="button" className="tileset-view-action" onClick={() => openViewer('loading')}>Open Loading Lab</button>
    ...
    },
    {
      ...
      controls: (
        <>
          ...
          {/* The Rail Lab tunes the rail families this same chrome inherits, so it belongs beside
              it. Registered and implemented, but openable only by typing ?vk= until now. */}
          <button type="button" className="tileset-view-action" onClick={() => openViewer('raillab')}>Open Rail Lab</button>
        </>
      ),
    },
    {
    ...
    {
      id: 'deployment', label: 'Deployment Lab', hint: 'Exercise the canonical Run placement phase across rows, dedicated regions, obstacles, rosters, abilities, seeds, and layout rolls.',
      main: <DeploymentLabCatalog onOpen={() => openViewer('deployment')} />,
    ...
    },
    {
      ...
      controls: <p className="tileset-view-note">Candidates only. Accepting a card is done from Card Prompts, one card at a time.</p>,
    },
    {
  ...
  const backToSceneMap = (): void => {
    const scene = SCENE_ANIM_SCENES.find((s) => s.regionIds.includes(selectedRegionId));
    if (!scene) throw new Error(`Selected scene animation "${selectedRegionId}" has no owning scene`);
