...
// The level editor lives at /edit. It uses the studio's clean catalog design
// language (the chosen system style) — header, calm dark panels, segmented
// controls — wrapping the socket-legal board. Paint terrain *families* and the
// solver lays down the legal tile per cell (base inside a region, transitions
// where families meet); place chess-piece units on top.
export function LevelEditorPage(): ReactElement {
  ...
  const summary =
    missingCount > 0
      ? `${missingCount} junction warning${missingCount === 1 ? '' : 's'}`
      : paintedCount > 0
        ? 'all edges legal'
        : 'empty board';

  return (
    <main className="tileset-studio-page level-editor-page" data-testid="level-editor">
      <header className="tileset-studio-header">
        <div className="tileset-studio-brand">
          ...
          <div className="tileset-studio-titleblock">
            <p className="tileset-studio-kicker">Level Editor</p>
            <h1>Untitled Level</h1>
            <p className="tileset-studio-subtitle">Paint terrain — the socket solver lays down legal tiles; place units on top.</p>
          </div>
        </div>
        <nav className="tileset-studio-actions" aria-label="Level editor navigation">
          <span className="tileset-mode-tabs" aria-label="Edit layer">
      ...
      <section className="tileset-studio-shell is-view" aria-label="Level editor">
        <div className="tileset-view-mode level-editor">
          <header className="tileset-view-header">
            <div className="tileset-view-headline">
              <h2>{layer === 'terrain' ? 'Terrain' : 'Units'}</h2>
              <p>{paintedCount} tile{paintedCount === 1 ? '' : 's'} · {unitCount} unit{unitCount === 1 ? '' : 's'} · {summary}</p>
            </div>
            <div className="tileset-view-header-actions" data-testid="editor-status" title={status}>
              ...
              <button type="button" onClick={validate} title="Validate the level (build + check the durable doc).">Test</button>
              <button type="button" className="is-primary" disabled title="Saving unlocks on the hosted environment.">Save</button>
            </div>
          </header>

          <div className="level-stage" onPointerDownCapture={beginStroke} onPointerUpCapture={endStroke}>
          ...
          <aside className="tileset-view-controls" aria-label="Level editor controls">
            <section className="tileset-inspector-section">
              <h2>Controls</h2>
              <div className="tileset-control-stack">
                ...
                {layer === 'terrain' ? (
                  <>
                    <p className="tileset-group-label">Terrain</p>
                    ...
                    <div className="tileset-button-row">
                      {(['small', 'wide'] as const).map((key) => (
                        <button key={key} type="button" className={sizeKey === key ? 'is-active' : ''} onClick={() => changeSize(key)} title={`${levelSizes[key].cols} × ${levelSizes[key].rows} board`}>
                          {levelSizes[key].cols} × {levelSizes[key].rows}
                        </button>
                      ))}
                    </div>
                    <div className="tileset-button-row">
