...
function preserveCardLayoutRouteParams(params: URLSearchParams, route: TilesetStudioRouteState): void {
  ...
  const current = new URLSearchParams(window.location.search);
  (['frameCandidate', 'coinCandidate', 'artCandidate', 'propertyCandidate', 'unitStateCandidate', 'cardVariant', 'contentsStudy', 'tacticalSpecimen', 'concinnousTarget', 'starterCard', 'rarityStudy', 'uncommonCandidate', 'rareCandidate', 'backStudy'] as const).forEach((key) => {
    const value = current.get(key);
    if (value) params.set(key, value);
  });
}
