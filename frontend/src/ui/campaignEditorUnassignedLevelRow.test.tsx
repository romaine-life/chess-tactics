...
describe('Campaign Editor unassigned level row', () => {
  it('offers edit and delete without campaign-order controls when the level is manageable', () => {
    ...
    expect(markup).toContain('data-chrome-unit="inner-box" class="inner-box inner-chrome-box settings-row action-list-row ce-editor-level-row active is-active is-selected is-neutral"');
  ...
  it('does not expose mutating controls while the level tier is unavailable', () => {
    ...
    expect(markup).toContain('data-chrome-unit="inner-box" class="inner-box inner-chrome-box settings-row action-list-row ce-editor-level-row is-read-only is-neutral"');
    expect(markup).toContain('aria-label="Preview Standalone map"');
