...
const levelSignature = (level: Level): string =>
  `${level.boardCode ?? ''}|${level.objective}|${level.placement ?? 'fixed'}|${level.surviveTurns ?? ''}|${JSON.stringify(level.roster ?? {})}|${level.timeControl ? `${level.timeControl.initialSeconds}+${level.timeControl.incrementSeconds}` : ''}`;
...
export function LevelEditor(): ReactElement {
  ...
  // The battle clock — off by default; when on, the level carries a TimeControl and the
  // skirmish runs the player's chess clock (the enemy is untimed).
  const [clockEnabled, setClockEnabled] = useState(false);
  const [clockInitialSeconds, setClockInitialSeconds] = useState<number>(DEFAULT_TIME_CONTROL.initialSeconds);
  const [clockIncrementSeconds, setClockIncrementSeconds] = useState<number>(DEFAULT_TIME_CONTROL.incrementSeconds);
