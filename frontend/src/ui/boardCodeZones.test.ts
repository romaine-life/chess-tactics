...
describe('boardCode — zones wire key (z)', () => {
  ...
  it('round-trips the King zone and a Player Deployment zone that bars the King', () => {
    const board = base({
      zoneEntries: [
        { id: 'zone-1', name: 'Player Deployment', color: 'blue', type: 'player-spawn', excludedPieceTypes: ['king'], tiles: ['0,0', '1,0'] },
  ...
  it('drops a type bar written on a zone type that cannot use one', () => {
    const board = base({ zoneEntries: [{ id: 'zone-1', type: 'region', excludedPieceTypes: ['king'], tiles: ['0,0'] }] });
    const decoded = decodeBoard(encodeBoard(board))!;
    expect(decoded.zoneEntries).toEqual([{ id: 'zone-1', type: 'region', tiles: ['0,0'] }]);
  });
  ...
  it('folds duplicate deployment zones into one per type, keeping every painted square', () => {
    ...
    // and no square it painted may be lost (ADR-0367).
    const board = base({
      zoneEntries: [
  ...
  it('does not revive the retired pawn-only bar spelling', () => {
    ...
    expect(decodeBoard(code)!.zoneEntries).toEqual([
      { id: 'zone-1', type: 'player-spawn', tiles: ['0,0'] },
    ]);
  });
