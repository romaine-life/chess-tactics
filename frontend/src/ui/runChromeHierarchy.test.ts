...
describe('Run chrome hierarchy', () => {
  ...
  it('fills shell-owned Run destinations while Deployment uses the battlefield', () => {
    ...
    expect(runExpunctioWorkspace).toContain('<KitScroll className="run-expunctio-list">');
    ...
    expect(runExpunctioWorkspace).toContain('data-chrome-fill-surface="hybrid-wood-oak"');
    ...
    expect(styleCss).toMatch(/\.run-expunctio-list > \.kit-scroll-content\s*\{[\s\S]*?padding-block:\s*var\(--ds-space-1\) 0;/);
    expect(styleCss).toMatch(/\.run-expunctio-list > \.kit-scroll-rail\s*\{[\s\S]*?bottom:\s*var\(--le-inner-atom-bottom-overhang, 0\);/);
    expect(styleCss).toMatch(/\.run-expunctio-workspace \.run-expunctio-gallery \.run-expunctio-row\.inner-box\s*\{[\s\S]*?overflow:\s*clip !important;[\s\S]*?overflow-clip-margin:\s*var\([\s\S]*?--le-inner-atom-bottom-clip-apron,[\s\S]*?var\(--le-inner-atom-bottom-overhang, 0px\)[\s\S]*?\);/);
    expect(styleCss).toMatch(/\.run-expunctio-gallery\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/);
