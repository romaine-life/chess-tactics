...
//   fn?:{cell:fenceMaterial}, rc?:[edgeKey], rx?:[edgeKey], z?:{cell:zoneType} }. `f` fills every
// cell, then `t` overrides — so a "mostly one tile" board stays tiny; `h` punches intentional holes
// back out of that fill. Features split per kind on the wire (rd=roads, rv=rivers, br=bridges,
// fn=fences) and merge into one `features` map on decode; bridges carry an explicit orientation
// ('h'/'v') instead of a connection (they don't autotile), so `br` stores that and `bm` carries a
// non-default deck material. `rc` (severed edges) and `rx` (forced outward exits) are shared edge
// lists. `z` is the gameplay-zone channel (ADR-0050): each painted cell -> its zone type. base64url
// of the JSON (no padding, +/ -> -_).
//
// FORWARD/BACK-COMPAT: `z`/`p`/`br` are emitted only when non-empty, so a board without them
...
import type { FeatureKind, FeatureMaterial, RoadMaterial, RiverMaterial, BridgeOrientation, FenceMaterial } from '../core/featureAutotile';
...
export function encodeBoard(b: EditorBoard): string {
  ...
  // bridges instead store their orientation ('h'/'v') in `br`, and only a NON-default deck material
  // in `bm` (with a single stone deck today, `bm` stays empty — kept for forward-compat variants).
...
export function decodeBoard(code: string): EditorBoard | null {
  try {
    ...
    // Merge the per-kind wire maps back into one features map (rd=roads, rv=rivers, br=bridges, fn=fences).
