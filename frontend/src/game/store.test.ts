...
describe('skirmish store', () => {
  ...
  it('walks an enemy piece on the player turn, and hands the move straight back', () => {
    ...
    expect(state.log[0].text).toBe("Admin Free Move — the enemy's Pawn to 2,2.");
    expect(state.adminMode).toBeNull();
  });
  ...
  it('writes them on the enemy reply too, and seats one marker per gold notice', () => {
    ...
    expect(useSkirmish.getState().log.map((entry) => entry.text)).toContain('Roland answers the call and takes the field.');
...
describe('skirmish store: survive + reach objectives', () => {
  ...
  it('reach: a PAWN promoting on a target cell wins after the promotion choice', () => {
    ...
    // Answerable in the same tick — no timer runs between the move and the question (ADR-0556).
...
describe('skirmish store: premoves', () => {
  ...
  it('opens a played promotion question in the same tick as the move, mid-glide', () => {
    ...
    // (ADR-0556). The canonical board has not moved it — that waits on the answer.
