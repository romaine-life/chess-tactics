...
describe('critical live-content startup ordering', () => {
  it('hydrates media then complete prop seats before importing or rendering App', () => {
    expect(source).not.toMatch(/import\s+\{\s*App\s*\}\s+from\s+['"]\.\/ui\/App['"]/);
    const media = source.indexOf('Promise.all([loadLiveMediaCatalog(), loadLiveUnitCatalog()])');
