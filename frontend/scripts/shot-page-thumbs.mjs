...
// in-capture via a fractional deviceScaleFactor. Each temporary WebP is uploaded
// directly as a live-media candidate and then deleted.
//
// Usage:
//   node scripts/shot-page-thumbs.mjs [--base URL] --slot-prefix ui/pages -- <upload options>
//
...
const slotPrefix = optionValue(toolArgs, '--slot-prefix').replace(/^\/+|\/+$/g, '');
if (!slotPrefix || !uploadArgs.length) {
  throw new Error('shot-page-thumbs requires --slot-prefix and live-media upload options after --');
}
...
const outDir = mkdtempSync(join(tmpdir(), 'page-thumbs-'));
...
try {
  for (const { name, route } of PAGES) {
    ...
    const slot = `${slotPrefix}/${name}.webp`;
    uploadGeneratedCandidate(out, uploadArgs, slot);
    console.log(`${name.padEnd(16)} ${route.padEnd(16)} -> ${slot} (${(size / 1024).toFixed(1)} KB)`);
    await page.close();
  }
} finally {
  await browser.close();
