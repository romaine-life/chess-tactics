@echo off
rem Runs ONCE, when a worktree's environment is created — Codex at worktree
rem creation, Claude via the chess-dev-start skill at session start. A worktree
rem hosts exactly one session, so there is never a prior environment to detect.
rem
rem Do not add guards, session ownership, or "is it already running" checks. That
rem was tried (PRs #630/#638) against a case that cannot occur, and reverted.
