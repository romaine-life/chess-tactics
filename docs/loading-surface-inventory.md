...
Status: audited 2026-07-19 against `origin/main` at `b5e6a5c9`. PR #507 now carries
the unmerged startup-font, Play DOM-surface, and complete playable-surface corrections.
...
| SYS-08 | Resource sharing | Board terrain and scene renderers share `imageResources`; most UI `<img>` nodes and local `new Image()` probes do not. Audit found 90 direct UI `<img>` sites and 11 imperative `new Image()` sites. PR #507 adds a reusable DOM boundary that inventories rendered `<img>`, background, border, and mask URLs through that shared cache. | **PARTIAL; Play/HUD migrated in PR #507** | P1 | Migrate remaining critical UI surfaces. Plain `<img>` remains valid only for progressive/noncritical content with stable geometry. |
...
| SUR-02 | `/` main-menu background, title, and primary buttons | `coldReveal` waits for the exact DB-installed animated-scene background consumed by the DOM plus title/button resources. The cold verifier samples the group every frame and rejects reveal unless all rendered critical images are complete. | One atomic reveal or explicit full-screen retry; exact cold Slow-4G capture is complete. | **PASS in PR #507; FAIL on main** | P0 | Merge and production-verify. Keep the verifier coupled to the rendered consumers whenever first-frame menu art changes. |
...
| SUR-04 | Main-menu destination column | Settings/Play/Lobbies render eagerly; Campaign Editor uses a local Suspense boundary whose fallback is an empty destination column. PR #507 corrects the DB-installed Play route to `/play/select/skirmish` and verifies cold Slow-4G click continuity without detaching the homepage backdrop. | Play remains in the menu family and reveals through its owned surface; the lazy Editor column can still be visibly empty. | **PARTIAL; Play fixed in PR #507** | P0 | Apply the same owned destination boundary to the lazy Campaign Editor column. |
| SUR-05 | `/play/select/skirmish` and `/play/select/levels` data | Main uses a local `loading` Boolean. PR #507 places the complete Play destination behind `PaintedSurfaceBoundary`, waiting for canonical hydration, rendered image/CSS resources, and two paint opportunities. | One status or retryable error is visible; all destination columns remain hidden and inert until complete. | **PASS in PR #507; FAIL on main** | P0 | Merge and production-verify the exact selector modes. |
| SUR-06 | `/play/select/campaign/:id` visible thumbnail group | Canonical derivatives retain their local group gate. PR #507 adds the owning Play boundary around data, rail, selected panel, direct images, CSS images, and thumbnail group. | Complete Play destination reveals once; failures have in-place retry. | **PASS in PR #507** | P0 | Merge and production-verify. Then narrow long-list critical manifests to the first viewport without weakening the group guarantee. |
...
| SUR-09 | `/play` canonical campaign/level resolution | `boardSettled` prevents placeholder mounting. PR #507 additionally couples board compositor readiness, HUD image/CSS readiness, title controls, interaction, and clock activation into one playable-surface token. | Cold route shows a deliberate battlefield status; soft navigation remains covered; the clock cannot run during loading. | **PASS for the canonical player route in PR #507** | P0 | Merge and production-verify. Cold generic lazy fallback remains a system-level gap for other direct routes. |
...
| SUR-12 | Gameplay HUD, title-bar controls, portraits, and icons | PR #507 mounts HUD content hidden, inventories direct and CSS-sourced images, waits for decode plus paint, and withholds board reveal, title status, interaction, and clock until both board and HUD are complete. | Board and complete gameplay chrome reveal together; failure cannot silently start play. | **PASS in PR #507; FAIL on main** | P0 | Merge and production-verify under cold Slow 4G. |
