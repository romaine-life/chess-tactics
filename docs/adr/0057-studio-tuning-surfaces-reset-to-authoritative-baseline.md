---
status: "accepted; committed asset-baseline clause superseded by ADR-0085"
date: 2026-07-03
deciders: Nelson, Claude
---

# ADR-0057: Every Studio tuning surface ships Reset-to-authoritative-baseline
...
the **authoritative published baseline**, and it is wired as part of building the
surface — not after the owner asks for it (again).
...
Chosen: **every Studio tuning surface ships a Reset-to-authoritative-baseline
control, wired before the surface ships.**

1. **Reset restores the authoritative baseline** — the values the surface
   currently publishes, so an untouched-then-reset panel is pixel-/behaviour-
   identical to production. That source is committed configuration for code-backed
   tuners and live storage for live-content tuners such as Unit Art. Zero-out ≠ reset.

2. **The baseline is derived, never hand-copied.** Reset reads from the same
   source of truth the surface renders from (published live data or shipped
   configuration), so it cannot drift from what actually ships. A hand-pasted "defaults"
...
4. **Reset is per-control, at the selected-item grain — not only a whole-surface
   ...
   back to that control's authoritative value, in addition to any "Reset all". A
