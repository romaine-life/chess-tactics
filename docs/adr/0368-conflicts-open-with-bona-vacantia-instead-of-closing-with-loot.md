...
A Run's lipsana used to arrive at the **end** of a Conflict. Beating a loot Battle rolled
...
and then spent gold in the same screen — the lipsanon was a reward, and it combined with the
...
and are not explained. The lipsanon flavor texts already lived there — "It was counted with the
...
**A Conflict opens with a Bona Vacantia screen: three lipsana, one taken, before the shop
...
- **Before the shop, not after.** The player inherits the lipsanon and *then* decides what to
  spend on. Shop-then-lipsanon would make the opening shop's fixed gold feel like it came
...
- **Derived from loot Battles, not from Battle numbers.** A Conflict already means "the
  ...
  the War. A Conflict opens with a lipsanon when there is a loot Battle at or after its first
  ...
  Battle 10 lipsanon-free — without hardcoding any of those numbers.
- **Moved, not added.** The loot lipsanon is gone from the shop entirely; `lootLipsanonOffers`
  and `chosenLootLipsanonId` are removed rather than left dead. Three lipsanon events a Run burns
  nine of twenty lipsana, and reveal is burn-on-sight. Six events would burn eighteen of a
  pool that is effectively nineteen, and After-Hours Key's per-Conflict paid lipsanon pushes
  past the end of it. Keeping both is a content problem, not a balance one.
- **Still mandatory.** There is no way past the screen without taking a lipsanon, exactly as
  the loot shop could not be left without one. Taking is also what opens the shop behind it.

The Run is **front-loaded** by this. A lipsanon won at the end of Conflict 1 was active for
...
*before* it, so a lipsanon taken there may already have paid out, and the pinned
`entrySnapshot.goldTenths` is no longer literally 80. It is pinned to 80 plus what the held
lipsana are worth on acquisition, computed from `RUN_LIPSANON_IMMEDIATE_GOLD` — one table read
...
(`ui/run/bona-vacantia/mat.png`), the mat drawn at 1.74x the lipsanon row. Lipsana sit on it
