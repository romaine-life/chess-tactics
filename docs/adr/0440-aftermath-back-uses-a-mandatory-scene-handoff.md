...
# ADR-0440: Aftermath Back uses a mandatory scene handoff

## Context

ADR-0438 made the exact terminal board reviewable from aftermath, but the report rendered Back
...
- Aftermath resolves Back from that handoff first. A reload may instead resolve the same exact
  ...
  Level identity already required by ADR-0438.
