...
# ADR-0054: Victory conditions are a two-list win/lose condition model
...
The expander reproduces the pre-ADR-0054 semantics exactly. The direction-aware `kingSide`
...
have survived). The owner confirmed this reading. It also preserves the pre-ADR-0054
...
The owner's rule: reach is *a pawn reaching the reach zone*. The pre-ADR-0054 code counted
...
- Authored `victory` flows level → save → skirmish through one new resolved-rules field on
  the store (`SkirmishState.victory`); preset and legacy levels expand to identical results.
- The **authoring UI is out of scope for this ADR** — the Level Editor cannot yet write a
  `victory` block; this ADR lands the rules engine (schema, expander, evaluator, validation,
  store wiring) so hand-authored / API bodies play correctly and the editor half has a stable
