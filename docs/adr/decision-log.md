...
| [0029](0029-nine-slice-line-frames-for-surface-fill.md) | A surface behind kit chrome uses a transparent-interior "line" frame (the FULL ornament/rail — assembled from the same atoms with a transparent fill, exterior carved; no masking) + the surface as a separate border-box background; every framed element keeps its frame over a surface; never a per-material baked frame; opt-in via a registry `line` field, parity-tested | accepted | 2026-06-27 |
