...
| [0030](0030-scrollbars-never-vanish.md) | Scrollbars never vanish: always-visible bare rail (`overflow:scroll`, a recessed groove even when empty), thumb only when scrollable (no false-affordance thumb); native scrollbar skinned with the chosen grip via a shared `.kit-scroll` (reset `scrollbar-color`/`width` to auto or Chrome drops the webkit skin; clip rounded containers); Chrome-target, Firefox `@supports` fallback | accepted | 2026-06-27 |
