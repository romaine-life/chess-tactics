...
| [0029](0029-nine-slice-line-frames-for-surface-fill.md) | A surface behind kit chrome uses a transparent-interior "line" frame (ornament only — baked from the same atoms with the fill dropped + navy masked, so the 9-slice edge slices don't ring it) + the surface as a separate border-box background; never a per-material baked frame; opt-in via a registry `line` field, parity-tested | accepted | 2026-06-27 |
