...
The first Enchiridion relic browser rendered every relic as a full-width
...
the row boundaries. The selected relic then stretched one detail frame through
the remaining pane despite having only a short description and two statistics.

## Decision

- Enchiridion and Strategikon relic references browse relics through one compact
  grid of native-size artwork.
- Each selectable relic consumes the canonical `inner-asset-swatch` primitive,
  not `inner-list-row`. The active swatch uses the shared selected state.
- Relic names and complete effects appear immediately through the shared styled
  tooltip on hover and keyboard focus. The interactive swatch itself remains
  the sole focus and activation target.
- The selected relic record contains the native icon, name, effect, history
...
- Narrow layouts stack the reliquary above the record; wide layouts place them
  side by side. Both hosts continue to use the same `RelicCodex` implementation.

## Consequences

- Relic artwork becomes the browsing language, matching how relics are
  encountered during a Run.
- More relics remain visible without a tall scrollbar or repeated clipped
  labels.
