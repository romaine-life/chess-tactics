...
# ADR-0256: Individual relics are routable from the main-menu Enchiridion

## Context

The main-menu Enchiridion's relic reference (ADR-0252–0254) selected a relic
...
individually selected relic was not: a reload, a shared link, sign-in
`returnTo`, and browser Back all forgot which relic record was open.

The scene director keys transitions off the leaf manifest id, which previously
embedded the full route path. A naïve per-relic path would therefore have
turned every relic click into an exit/enter veil cycle, degrading what ADR-0253
established as direct, instant selection.

## Decision

- Each relic has a main-menu address: `/enchiridion/relics/<relic-id>`, built
...
- In the main-menu host, relic-reference entries are NavButtons to those
  addresses (ADR-0052): selection pushes the relic's route, so reload, Back /
  Forward, sign-in return, and shared links restore the same open record.
- Relic addresses are the SAME retained scene as `/enchiridion/relics`: the
  ...
  relic instance carries no per-relic param. Selecting a relic is an
  address-only update inside the committed scene — no veil, no slot re-key;
  section changes remain real scene transitions.
- An unknown or absent relic id renders the section's default selection (the
  first visible relic) without rewriting the URL, mirroring the section
  fallback to `units`.
- The Battle-hosted Strategikon keeps ephemeral local relic selection in both
  its Enchiridion and Lipsanotheca surfaces: mid-battle reference peeks do not
  write history. One `RelicTrigger` control renders both transports so the two
  hosts cannot drift visually (ADR-0231/0059).

## Consequences

- A specific relic record can be handed to the owner, revisited after reload,
  and walked with Back/Forward from the main menu.
- Relic browsing keeps ADR-0253's instant, tooltip-free selection feel; only
