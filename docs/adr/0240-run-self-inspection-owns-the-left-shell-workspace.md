...
partially_supersedes:
  - ADR-0216 relic-strip visibility while self-inspection is open
...
Play shell, leaving the relic overlay above it and making player inspection read
...
inspect relics once that workspace covers the persistent relic strip.
...
- Run has a primary operational destination for its current phase, a
  ...
  **Self inspection** group containing **Army** and **Relics**.
...
- The right Controls panel remains fixed. Its primary phase button returns to
  the operational destination; Army and Relics remain sibling destinations
  under the explicit **Self inspection** heading. Shop Reset, Continue, and
  Run abandonment retain their existing ownership.
- Opening either self-inspection destination suppresses the relic overlay,
  because that overlay occupies the workspace being replaced. **Relics**
  provides the durable replacement: a readable list of every held relic's
...
- Army and Relics read as two views of the player rather than shop or Battle
  controls.
...
- The relic strip remains persistent on operational and transactional Run
  destinations, but intentionally yields to the complete Relics workspace
