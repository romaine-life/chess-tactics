...
two-column lipsanon browser while leaving usable scenic canvas empty to the right.
...
the lipsanon list and selected record into it.
...
- The lipsanon browser uses its existing side-by-side layout when the resulting
...
- ADR-0253's tooltip decision remains in force: lipsanon-reference entries are
  direct selection buttons and the selected record is the sole visible
  description authority.

## Consequences

- Lipsanon details no longer clip against an invisible action-column boundary.
