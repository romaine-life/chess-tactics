...
authored sequence, receives shops and lipsana, and retries lost boards without ending the
Run. The feature must remain immediately recognizable as chess. The surrounding economy,
deployment, information, objectives, and rewards may change; a lipsanon must never give a
...
- A Loot shop reveals three previously unseen lipsana and grants exactly one for free.
  Merely revealing a lipsanon burns it from all later offers in that Run.
- Merchant's Shopkey adds one fixed 10-gold lipsanon offer to every shop in a Conflict. It is
...
### Initial lipsanon registry

The registry launches with the twenty owner-approved lipsana below and remains data-driven
...
13. **Merchant's Shopkey** — the persistent per-Conflict paid lipsanon offer above.
...
Lipsana persist for the Run unless their wording is Battle-only. Royal Tent's rocks are
...
- The pure piece rules remain the authority for legal movement, capture, attack, castling,
  ...
  may not branch legal move generation by lipsanon.
- Tests enumerate every lipsanon and fail if any lipsanon id is consumed by the movement/capture
  rule modules.
- Seeded War choice, draft, deployment, blocking, shops, and lipsanon offers are persisted or
...
- Twenty lipsana are usable at launch; seven more require owner approval to reach the
