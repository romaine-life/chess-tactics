...
BASE=a[0]; OUT=a[1]; ORTHO=float(a[2]); ZS=float(a[3]); TZ=float(a[4]); RH=int(a[5]) if len(a)>5 else 140
...
sc.render.resolution_x=96; sc.render.resolution_y=RH; sc.render.film_transparent=True
