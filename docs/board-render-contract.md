...
Run card formations implement [ADR-0515](adr/0515-player-arranges-rotation-canonical-cards-from-a-complete-shuffle.md)
in board space. Deployment hover previews and committed pieces use the canonical isometric
projection at the exact board cells selected by the player. Rotation changes board-relative
offsets only; it never reflects the formation or substitutes screen-space placement.
