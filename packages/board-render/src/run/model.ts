...
export interface RunAftermathState {
  ...
  goldTenths: number;
  /** The part of `goldTenths` a relic paid on top of the Battle's own reward. */
...
/**
 * What the Battle just fought pays out: its own reward, plus whatever a relic adds on top
...
function battleRewardTenths(run: RunDocument, survivingUnitIds: readonly string[]): {
  ...
  return {
    ...
    bonusGoldTenths: hasRelic(run, 'mercenarys-rifle')
