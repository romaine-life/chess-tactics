 ...
 * Sectio can hand them back to the Victory screen they came from (ADR-0567). Leaving for the
...
export function normalizeRunDocument(run: RunDocument): RunDocument {
  ...
  // back to the Victory it came from (ADR-0567). Leaving for the next Deployment retires it.
 ...
 * cards, so the review's way out is a return and never a repeat (ADR-0567).
 */
...
export function openSectio(
  ...
  // deciding what to spend it on (ADR-0567). Leaving the Sectio for the next Deployment ends it.
